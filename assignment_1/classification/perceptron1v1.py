	...
	def train( self, trainingData, trainingLabels, validationData, validationLabels, validate):
		...
		for iteration in range(self.max_iterations):
			...
			for i in range(len(trainingData)):
				...
				for label_ in self.legalLabels:
					if label_ == label:
						continue

					label1 = min(label, label_)
					label2 = max(label, label_)
					pos = (label == label2)

					if self.weights[frozenset([label1, label2])] * data > 0	and pos == False:
						self.weights[frozenset([label1, label2])] -= data
					elif self.weights[frozenset([label1, label2])] * data <= 0 and pos == True:
						self.weights[frozenset([label1, label2])] += data
